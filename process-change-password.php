...
try {
    ...
    $query = "SELECT user_id, password FROM users WHERE ( user_id=? )";
    ...
    if ((mysqli_num_rows($result) == 1)
       ...
       // use prepared statement to insure that only text is inserted
       // bind fields to SQL Statement
       mysqli_stmt_bind_param($q, 'si', $hashed_passcode, $user_id);
       // execute query
       mysqli_stmt_execute($q);
       if (mysqli_stmt_affected_rows($q) == 1) {	// one row updated
        ...
        $errorstring .= 'The password do not match those on record.';
